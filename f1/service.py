...
class F1Service:
    ...
    def _get_latest_finished_race(self, season_year: Optional[int] = None) -> Optional[Dict]:
        ...
        if not season_id:
            return None
        print("THE SEASON ID IS: ", season_id)

        stages = self._make_request(f"/stages/season/{season_id}", params={"include": "results"})
        if not stages:
            return None
    ...
    def get_last_race_results(
        ...
        for result in results:
            ...
            formatted_results.append({
                ...
                "constructor_id": str(team_id) if team_id is not None else "",
                "constructor_name": "",  # Not available in results, only team_id
                "points": 0.0,  # Points not available in race results payload
